...
# -*- coding: utf-8 -*-
"""
Created on Mon May 13 19:24:30 2024
...
aug_train_texts = augmented_data['texts']
aug_train_labels = augmented_data['labels']
...
def transformer_model(vocab_size, num_labels):
    ...
    # Transformer block with increased dropout and L2 regularization
    attn_output = MultiHeadAttention(num_heads=4, key_dim=128)(x, x)
    attn_output = Dropout(0.3)(attn_output)
    out1 = LayerNormalization(epsilon=1e-6)(x + attn_output)

    ffn_output = Dense(2048, activation='relu', kernel_regularizer=l2(0.1))(out1)
    ffn_output = Dense(128, activation='relu', kernel_regularizer=l2(0.1))(ffn_output)
...
# 保存模型
model.save('optimized_emotion_transformer_model.h5')
...
# 测试模型
test_loss, test_acc = model.evaluate(test_seqs, test_labels)
